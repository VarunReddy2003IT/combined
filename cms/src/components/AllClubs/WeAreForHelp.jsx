...
function WeAreForHelp() {
  // State variables
  const [isLeadForWeAreForHelp, setIsLeadForWeAreForHelp] = useState(false);
  ...
  useEffect(() => {
    if ((userRole === 'lead' && userClub === 'WeAreForHelp') || userRole === 'admin') {
      setIsLeadForWeAreForHelp(true);
    }
    fetchEvents();
    fetchClubData();
  }, [userRole, userClub]);
  ...
  const fetchClubData = async () => {
    try {
      const response = await axios.get("https://finalbackend-8.onrender.com/api/clubs/WeAreForHelp");
      if (response.data) {
        ...
        <div className="page-content">
          ...
          <div className="club-header">
            <div className="logo-container">
              {clubLogo ? (
                <img src={clubLogo} alt="WeAreForHelp Logo" className="club-logo" />
              ) : (
            ...
            <h1 className="page-title">WeAreForHelp</h1>
          </div>
          ...
          <div className="club-info-display">
            ...
            {clubDescription ? (
              ...
              <div className="club-description">
                <p className="empty-description">No description available. {isLeadForWeAreForHelp ? 'Click the edit button to add a description.' : ''}</p>
              </div>
            )}
            ...
            {labels && labels.length > 0 && labels.some(label => label.name || label.value) ? (
              ...
              <div className="club-labels">
                <p className="empty-labels">No information available. {isLeadForWeAreForHelp ? 'Click the edit button to add information.' : ''}</p>
              </div>
            )}
          </div>
