...
function CSI() {
  ...
  const fetchClubData = async () => {
    try {
      const response = await axios.get("https://finalbackend-8.onrender.com/api/clubs/CSI");
      if (response.data) {
  ...
  const fetchEvents = async () => {
    try {
      ...
      const upcomingEvents = CSIEvents.filter(event => event.date >= today);
      const pastEvents = CSIEvents.filter(event => event.date < today);
        ...
        <div className="page-content">
          ...
          <div className="club-header">
            <div className="logo-container">
              {clubLogo ? (
                <img src={clubLogo} alt="CSI Logo" className="club-logo" />
              ) : (
            ...
            <h1 className="page-title">CSI</h1>
          </div>
          ...
          <div className="club-info-display">
            ...
            {clubDescription ? (
              ...
              <div className="club-description">
                <p className="empty-description">No description available. {isLeadForCSI ? 'Click the edit button to add a description.' : ''}</p>
              </div>
            )}
            ...
            {labels && labels.length > 0 && labels.some(label => label.name || label.value) ? (
              ...
              <div className="club-labels">
                <p className="empty-labels">No information available. {isLeadForCSI ? 'Click the edit button to add information.' : ''}</p>
              </div>
            )}
          </div>
