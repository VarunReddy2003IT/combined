...
function YES() {
  // State variables
  const [isLeadForYES, setIsLeadForYES] = useState(false);
  ...
  useEffect(() => {
    if ((userRole === 'lead' && userClub === 'YES') || userRole === 'admin') {
      setIsLeadForYES(true);
    }
    fetchEvents();
    fetchClubData();
  }, [userRole, userClub]);
  ...
  const fetchClubData = async () => {
    try {
      const response = await axios.get("https://finalbackend-8.onrender.com/api/clubs/YES");
      if (response.data) {
  ...
  const fetchEvents = async () => {
    try {
      ...
      const YESEvents = response.data.filter(event => event.club === 'YES');
      const today = new Date().toISOString().split("T")[0];
      const upcomingEvents = YESEvents.filter(event => event.date >= today);
      const pastEvents = YESEvents.filter(event => event.date < today);
        ...
        <div className="page-content">
          ...
          <div className="club-header">
            <div className="logo-container">
              {clubLogo ? (
                <img src={clubLogo} alt="YES Logo" className="club-logo" />
              ) : (
            ...
            <h1 className="page-title">YES Club</h1>
          </div>
          ...
          <div className="club-info-display">
            ...
            {clubDescription ? (
              ...
              <div className="club-description">
                <p className="empty-description">No description available. {isLeadForYES ? 'Click the edit button to add a description.' : ''}</p>
              </div>
            )}
            ...
            {labels && labels.length > 0 && labels.some(label => label.name || label.value) ? (
              ...
              <div className="club-labels">
                <p className="empty-labels">No information available. {isLeadForYES ? 'Click the edit button to add information.' : ''}</p>
              </div>
            )}
          </div>
