...
function YFS() {
  // State variables
  const [isLeadForYFS, setIsLeadForYFS] = useState(false);
  ...
  useEffect(() => {
    if ((userRole === 'lead' && userClub === 'YFS') || userRole === 'admin') {
      setIsLeadForYFS(true);
    }
    fetchEvents();
    fetchClubData();
  }, [userRole, userClub]);
  ...
  const fetchClubData = async () => {
    try {
      const response = await axios.get("https://finalbackend-8.onrender.com/api/clubs/YFS");
      if (response.data) {
  ...
  const fetchEvents = async () => {
    try {
      ...
      const YFSEvents = response.data.filter(event => event.club === 'YFS');
      const today = new Date().toISOString().split("T")[0];
      const upcomingEvents = YFSEvents.filter(event => event.date >= today);
      const pastEvents = YFSEvents.filter(event => event.date < today);
        ...
        <div className="page-content">
          ...
          <div className="club-header">
            <div className="logo-container">
              {clubLogo ? (
                <img src={clubLogo} alt="YFS Logo" className="club-logo" />
              ) : (
            ...
            <h1 className="page-title">YFS</h1>
          </div>
          ...
          <div className="club-info-display">
            ...
            {clubDescription ? (
              ...
              <div className="club-description">
                <p className="empty-description">No description available. {isLeadForYFS ? 'Click the edit button to add a description.' : ''}</p>
              </div>
            )}
            ...
            {labels && labels.length > 0 && labels.some(label => label.name || label.value) ? (
              ...
              <div className="club-labels">
                <p className="empty-labels">No information available. {isLeadForYFS ? 'Click the edit button to add information.' : ''}</p>
              </div>
            )}
          </div>
