...
function NSS1() {
  // State variables
  const [isLeadForNSS1, setIsLeadForNSS1] = useState(false);
  ...
  useEffect(() => {
    if ((userRole === 'lead' && userClub === 'NSS1') || userRole === 'admin') {
      setIsLeadForNSS1(true);
    }
    fetchEvents();
    fetchClubData();
  }, [userRole, userClub]);
  ...
  const fetchClubData = async () => {
    try {
      const response = await axios.get("https://finalbackend-8.onrender.com/api/clubs/NSS1");
      if (response.data) {
  ...
  const fetchEvents = async () => {
    try {
      ...
      const upcomingEvents = NSS1Events.filter(event => event.date >= today);
      const pastEvents = NSS1Events.filter(event => event.date < today);
        ...
        <div className="page-content">
          ...
          <div className="club-header">
            <div className="logo-container">
              {clubLogo ? (
                <img src={clubLogo} alt="NSS1 Logo" className="club-logo" />
              ) : (
          ...
          <div className="club-info-display">
            ...
            {clubDescription ? (
              ...
              <div className="club-description">
                <p className="empty-description">No description available. {isLeadForNSS1 ? 'Click the edit button to add a description.' : ''}</p>
              </div>
            )}
            ...
            {labels && labels.length > 0 && labels.some(label => label.name || label.value) ? (
              ...
              <div className="club-labels">
                <p className="empty-labels">No information available. {isLeadForNSS1 ? 'Click the edit button to add information.' : ''}</p>
              </div>
            )}
          </div>
