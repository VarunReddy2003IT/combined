...
function AlgoRhythm() {
  ...
  const fetchClubData = async () => {
    try {
      const response = await axios.get("https://finalbackend-8.onrender.com/api/clubs/AlgoRhythm");
      if (response.data) {
  ...
  const fetchEvents = async () => {
    try {
      ...
      const AlgoRhythmEvents = response.data.filter(event => event.club === 'AlgoRhythm');
      const today = new Date().toISOString().split("T")[0];
      const upcomingEvents = AlgoRhythmEvents.filter(event => event.date >= today);
      const pastEvents = AlgoRhythmEvents.filter(event => event.date < today);
        ...
        <div className="page-content">
          ...
          <div className="club-header">
            <div className="logo-container">
              {clubLogo ? (
                <img src={clubLogo} alt="AlgoRhythm Logo" className="club-logo" />
              ) : (
            ...
            <h1 className="page-title">AlgoRhythm</h1>
          </div>
          ...
          <div className="club-info-display">
            ...
            {clubDescription ? (
              ...
              <div className="club-description">
                <p className="empty-description">No description available. {isLeadForAlgoRhythm ? 'Click the edit button to add a description.' : ''}</p>
              </div>
            )}
            ...
            {labels && labels.length > 0 && labels.some(label => label.name || label.value) ? (
              ...
              <div className="club-labels">
                <p className="empty-labels">No information available. {isLeadForAlgoRhythm ? 'Click the edit button to add information.' : ''}</p>
              </div>
            )}
          </div>
