...
function YouthForSeva() {
  // State variables
  const [isLeadForYouthForSeva, setIsLeadForYouthForSeva] = useState(false);
  ...
  useEffect(() => {
    if ((userRole === 'lead' && userClub === 'YouthForSeva') || userRole === 'admin') {
      setIsLeadForYouthForSeva(true);
    }
    fetchEvents();
    fetchClubData();
  }, [userRole, userClub]);
  ...
  const fetchClubData = async () => {
    try {
      const response = await axios.get("https://finalbackend-8.onrender.com/api/clubs/YouthForSeva");
      if (response.data) {
  ...
  const fetchEvents = async () => {
    try {
      ...
      const YouthForSevaEvents = response.data.filter(event => event.club === 'YouthForSeva');
      const today = new Date().toISOString().split("T")[0];
      const upcomingEvents = YouthForSevaEvents.filter(event => event.date >= today);
      const pastEvents = YouthForSevaEvents.filter(event => event.date < today);
        ...
        <div className="page-content">
          ...
          <div className="club-header">
            <div className="logo-container">
              {clubLogo ? (
                <img src={clubLogo} alt="YouthForSeva Logo" className="club-logo" />
              ) : (
                <div className="club-logo-placeholder">No Logo</div>
              )}
              {isLeadForYouthForSeva && (
                <button 
            ...
            <h1 className="page-title">YouthForSeva Club</h1>
          </div>
          ...
          <div className="club-info-display">
            ...
            {clubDescription ? (
              ...
              <div className="club-description">
                <p className="empty-description">No description available. {isLeadForYouthForSeva ? 'Click the edit button to add a description.' : ''}</p>
              </div>
            )}
            ...
            {labels && labels.length > 0 && labels.some(label => label.name || label.value) ? (
              ...
              <div className="club-labels">
                <p className="empty-labels">No information available. {isLeadForYouthForSeva ? 'Click the edit button to add information.' : ''}</p>
              </div>
            )}
          </div>
